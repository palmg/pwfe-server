...
var process = new function () {
    ...
    var execute = function execute(renderActions, cb) {
        ...
        _this.ctx.fluxStore.subscribe(function () {
            ...
            var count = _cache3.default.get(_count_prefix + route.id) ? _cache3.default.get(_count_prefix + route.id) : 0;
            ++count;
            _cache3.default.set(_count_prefix + route.id, count, _this.ctx.isCache.ttl);
    ...
    this.facade = new _facade2.default({
        ...
        cache: function cache(res, rej) {
            var route = _this.ctx.route;
            //获取dipatch次数和缓存做对比
            var count = route.renderActions && route.renderActions.dispathCount ? route.renderActions.dispathCount : 0;
            //如果cache中已经执行过，state已经被缓存起来
            count && _cache3.default.get(_count_prefix + route.id) === count ? res() : _render(res);
        }
    });
