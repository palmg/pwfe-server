...
console.log('static-copy', env.getParam('static').map(function (i) {
    return { from: i };
}));
...
function () {
    ...
    allChunks: true
}), new ProgressBarWebpackPlugin(), new webpack.NormalModuleReplacementPlugin(/\/iconv-loader$/, 'node-noop'), new CopyWebpackPlugin(env.getParam('static').map(function (i) {
    return { from: i };
}))],
    serverPlugins = [new webpack.optimize.OccurrenceOrderPlugin(), new webpack.DefinePlugin(defined), new ExtractTextPlugin({
    filename: env.getParam('cssFileName'),
    allChunks: true
}), new ProgressBarWebpackPlugin(), new webpack.NormalModuleReplacementPlugin(/\/iconv-loader$/, 'node-noop')];

env.getParam('compressJs') && function () {
